...
import { AnalysisResponse, GroundingSource, TechnicalDetailedRatings, FundamentalDetailedRatings } from '../types';
...
const AnalysisSection: React.FC<{
  ...
  type: 'technical' | 'fundamental';
}> = ({ icon, title, score, summary, detailedRatings, type }) => {
    ...
    // 詳細評価の項目名と日本語表示名のマッピング
    const technicalRatingLabels: Record<keyof TechnicalDetailedRatings, string> = {
        trend: 'トレンド',
        momentum: 'モメンタム',
        volatility: 'ボラティリティ',
        support: 'サポートレベル',
    ...
    const fundamentalRatingLabels: Record<keyof FundamentalDetailedRatings, string> = {
        profitability: '収益性',
        growth: '成長性',
        valuation: 'バリュエーション',
        financial: '財務健全性',
    ...
    const ratingLabels = type === 'technical' ? technicalRatingLabels : fundamentalRatingLabels;
      ...
      <div className="bg-gray-800/60 p-6 rounded-xl border border-gray-700 h-full flex flex-col">
        ...
        {detailedRatings && (
          <div className="mt-4 pt-4 border-t border-gray-700">
            <h4 className="text-sm font-semibold text-gray-300 mb-3">詳細評価</h4>
...
const AnalysisDisplay: React.FC<AnalysisDisplayProps> = ({ data, sources, canQuestionAnalysis = false, onQuestionAnalysis }) => {
  console.log('AnalysisDisplay data:', JSON.stringify(data));
  const { priceInfo, technicalAnalysis, fundamentalAnalysis, overallJudgement, questionAnswer } = data;

  return (
    <div className="space-y-4 md:space-y-6 spacing-responsive animate-fade-in-up">
      {/* Price Info */}
      {priceInfo && (
        <div className="price-grid grid grid-cols-2 md:grid-cols-3 gap-4 text-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-8">
          <AnalysisSection 
            ...
            detailedRatings={technicalAnalysis?.detailedRatings as TechnicalDetailedRatings} 
            type="technical" 
          />
          <AnalysisSection 
            icon={<BookOpenIcon className="h-8 w-8 text-blue-400"/>} 
            title="ファンダメンタル分析" 
            score={fundamentalAnalysis?.score} 
            summary={fundamentalAnalysis?.summary} 
            detailedRatings={fundamentalAnalysis?.detailedRatings as FundamentalDetailedRatings} 
            type="fundamental" 
          />
        </div>
        <div className="lg:col-span-2 bg-gray-800/60 p-6 rounded-xl border border-gray-700 flex flex-col justify-center items-center">
            ...
            {technicalAnalysis && fundamentalAnalysis ? (
              (() => {
                const tech = technicalAnalysis.detailedRatings as TechnicalDetailedRatings | undefined;
                const fund = fundamentalAnalysis.detailedRatings as FundamentalDetailedRatings | undefined;
                const toPct = (v?: number) => (v ? v * 20 : 0);
                const metrics = {
                  trend: toPct(tech?.trend),
                  volatility: toPct(tech?.volatility),
                  liquidity: toPct(tech?.momentum),
                  profitability: toPct(fund?.profitability),
                  growth: toPct(fund?.growth),
                  valuation: toPct(fund?.valuation),
                };

                return (
                  <>
                    <ScoreChart metrics={metrics} />
                    <ul className="text-gray-400 text-xs mt-4 space-y-1 list-disc list-inside text-left">
                      <li>トレンド強度: 直近の上昇・下降の勢い</li>
                      <li>ボラティリティ: 値動き幅の大きさ</li>
                      <li>流動性: 出来高など売買しやすさ</li>
                      <li>収益性: ROE・利益率など</li>
                      <li>成長性: 売上・EPS の伸び</li>
                      <li>割安度: PER / PBR 等による割安さ</li>
                    </ul>
                  </>
                );
              })()
       ...
       {overallJudgement ? (
        <div className="bg-gray-800/60 p-6 rounded-xl border border-gray-700">
            ...
            <ul className="text-gray-400 leading-relaxed mt-4 space-y-2">
              {overallJudgement.summary.split(/[。．]/).filter(p=>p.trim().length>0).map((p,idx)=>(
                <li key={idx} className="flex items-start"><span className="text-blue-400 mr-2 mt-1">•</span><span>{p.trim()}</span></li>
              ))}
              <li className="flex items-start"><span className="text-blue-400 mr-2 mt-1">•</span><span>最終的な投資判断はご自身の責任でお願いいたします。</span></li>
            </ul>
        </div>
      ) : (
